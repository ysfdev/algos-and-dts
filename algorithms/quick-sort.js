...
class QuickSort {
    ...
    recQuickSort(left, right) {
        ...
        if (size <= 3) { //use manualSort for small arrays
            this.manualSort(left, right);
        } else { //use recQuickSort
    ...
    medianOf3(left, right) {
        ...
        if (this.array[left] > this.array[center]) //order left and center
            this.swap(left, center);
        if (this.array[left] > this.array[right]) //order left and right
            this.swap(left, right);
        if (this.array[center] > this.array[right]) //order center right
    ...
    manualSort(left, right) {
        ...
        if (size === 2) {
            ...
            if (this.array[left] > this.array[right-1]) //left, center
                this.swap(left, right -1);
            if (this.array[left] > this.array[right]) //left, right
                this.swap(left, right);
            if (this.array[right -1] > this.array[right]) //center, right
